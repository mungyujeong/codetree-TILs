    ...
    for (auto p : v) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if ((pos[cur] + d == pos[i]) && 
                (idx[cur] == idx[i])) 
                return 0; // 이미 있음
            if (board[idx[i]][pos[i]] == 25 && board[idx[cur]][pos[cur] + d] == 25 || 
                board[idx[i]][pos[i]] == 30 && board[idx[cur]][pos[cur] + d] == 30 ||
                board[idx[i]][pos[i]] == 35 && board[idx[cur]][pos[cur] + d] == 35 ||
                board[idx[i]][pos[i]] == 40 && board[idx[cur]][pos[cur] + d] == 40) 
                return 0;
        }
